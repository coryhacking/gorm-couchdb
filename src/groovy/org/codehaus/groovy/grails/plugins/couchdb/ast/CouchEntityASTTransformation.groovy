 ...
 */
package org.codehaus.groovy.grails.plugins.couchdb.ast
	...
	private void injectIdProperty(ClassNode classNode) {
		...
		if (nodes) {
			...
			if (identity) {
				...
				// don't inject if we inherit this property
				if (GrailsASTUtils.hasOrInheritsProperty(classNode, IDENTITY)) {
					return
				}
	...
	private void injectJSONPropertyAnnotations(ClassNode classNode) {
		...
		for (MethodNode method: classNode.getMethods()) {
			...
			if (!method.isPrivate() && (getterName.startsWith("get") || (getterName.startsWith("is") && method.getReturnType() == BOOLEAN_TYPE))) {
				...
				if (!getLocalSetterMethod(classNode, setterName)) {
